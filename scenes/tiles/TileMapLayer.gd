...
var moisture = FastNoiseLite.new() # генератор шума
...
func generate_chunk(pos):
	for x in range(width):
		for y in range(height):
			...
			if alt < 0:
				# Arbitrary sea level value
				set_cell(
					Vector2i(pos.x - (width / 2) + x, pos.y - (height / 2) + y),
					0,
					Vector2i(2, roundi(3 * (temp + 10) / 20))
				)
			else:
				# Land tiles
				set_cell(
